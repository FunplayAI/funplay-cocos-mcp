...
class McpServer {
  ...
  async start() {
    ...
    const requestHandler = async (request, response) => {
      try {
        ...
        const authError = this.validateAuthentication(request, rpc);
        if (authError) {
          return json(response, authError.statusCode, authError.error, responseProtocolVersion);
        }
    ...
    while (attempt <= MAX_PORT_FALLBACK_ATTEMPTS) {
      ...
      try {
        ...
        this.log('info', `Authentication token (required for tool calls; keep private): ${this.authToken}`);
        return;
      } catch (error) {
        lastError = error;
  ...
  validateAuthentication(request, rpc) {
    ...
    return {
      statusCode: 401,
      error: this.createError(rpc && rpc.id, -32001, 'Unauthorized: missing or invalid authentication token.'),
    };
  }
  ...
  isAuthorizedRequest(request) {
    ...
    const providedToken = Buffer.from(provided.slice(7));
