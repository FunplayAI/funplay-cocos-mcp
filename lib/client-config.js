...
function getClaudeScopePath(projectPath, platform = process.platform) {
  const resolved = findGitRootOrSelf(projectPath);
...
function buildTargets(config, options = {}) {
  ...
  return [
    ...
    ...(target.id === 'claude_code' ? { scopePath: getClaudeScopePath(config.projectPath, platform), platform } : {}),
